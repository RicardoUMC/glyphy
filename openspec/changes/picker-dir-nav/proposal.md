...
- Track current working directory in App state
- Visual distinction: directories shown before files, colored differently

## Approach
1. Add `picker_cwd: PathBuf` to App to track current directory
...
5. On Enter: if directory → change CWD + re-scan; if file → select as before
...
## Open Questions
- Should we use `std::env::set_current_dir` or maintain a virtual path? → Use set_current_dir (simpler, matches user expectation)
